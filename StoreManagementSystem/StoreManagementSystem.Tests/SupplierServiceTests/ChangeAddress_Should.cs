...
{
    [TestClass]
    public class ChangeAddress_Should
    {
        [TestMethod]
        public void Succeed_If_SupplierExists()
        {
            ...
            using (var assertContext = new ShopContext(TestUtils.GetOptions(nameof(Succeed_If_SupplierExists))))
            {
                ...
                var updatedSupplier = sut.ChangeAddress(supplier, "Some New address");

                Assert.AreEqual(updatedSupplier.Address.ToString(), "Some New address");
